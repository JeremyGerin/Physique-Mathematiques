...
\begin{definition}
    ...
    $$\forall x, y\in \mathfrak{g}, \quad \kappa(x,y) = \tr(\ad_x \ad_y)$$
\end{definition}
...
\subsection{Extensions d'algèbres de Lie}

\begin{definition}
    Une \emph{extension} d'une algèbre de Lie $\mathfrak{h}$ par une algèbre de Lie $\mathfrak{n}$ est une algèbre de Lie $\mathfrak{g}$ telle qu'on ait la suite exacte courte d'algèbres de Lie
    \begin{diag}
    0 \arrow[r] & \mathfrak{n}  \arrow[r, "i"] & \mathfrak{g}  \arrow[r, "\pi"] & \mathfrak{h}  \arrow[r] & 0
    \end{diag}
    L'extension est dite \emph{triviale} s'il existe un isomorphisme $\varphi : \mathfrak{g} \to \mathfrak{n} \oplus \mathfrak{h}$ tel que le diagramme suivant commute
    \begin{diag}
        {} & {} & \mathfrak{g} \arrow[rd, "\pi"] \arrow[dd, "\varphi"] & {} & {} \\
        0 \arrow[r] & \mathfrak{n} \arrow[ru, "i"] \arrow[dr, "\iota_1"'] &  & \mathfrak{h} \arrow[r] & 0 \\ 
        & & \mathfrak{n}\oplus \mathfrak{h} \arrow[ur, "\pi_2"'] & &
    \end{diag}
    L'extension est dite \emph{scindée} s'il existe un morphisme $s : \mathfrak{h} \to \mathfrak{g}$ tel que 
    \begin{diag}
    0 \arrow[r] & \mathfrak{n}  \arrow[r, "i"] & \mathfrak{g}  \arrow[r, "\pi"] & \mathfrak{h}  \arrow[r]\arrow[l, dashed, "s", bend left] & 0
    \end{diag}
    L'extension est dite \emph{centrale} si $i(\mathfrak{n}) \subset \mathcal{Z}(\mathfrak{g})$.
\end{definition}

\begin{proposition}
    Une extension tiviale est scindée.
\end{proposition}

\omitted

\subsection{Produit semi-direct d'algèbres de Lie}

\begin{definition}
    Soit $\mathfrak{h}$ et $\mathfrak{n}$ deux algèbres de Lie et soit $\phi: \mathfrak{h} \to \Der(\mathfrak{n})$ un morphisme d'algèbres de Lie. Le \emph{produit semi-direct} de $\mathfrak{n}$ par $\mathfrak{h}$ relativement à $\phi$ est l'algèbre de Lie $\mathfrak{n} \rtimes_\phi \mathfrak{h}$ définie sur le $\K$-espace vectoriel $\mathfrak{n} \times \mathfrak{h}$ par le crochet de Lie
    $$[(n_1, h_1), (n_2, h_2)] = ([n_1, n_2]_{\mathfrak{n}} + \phi(h_1)(n_2) - \phi(h_2)(n_1), [h_1, h_2]_{\mathfrak{h}})$$
\end{definition}

\begin{proposition}
    Une extension d'algèbre de Lie 
    \begin{diag}
    0 \arrow[r] & \mathfrak{n}  \arrow[r, "i"] & \mathfrak{g}  \arrow[r, "\pi"] & \mathfrak{h}  \arrow[r] & 0
    \end{diag}
    est scindée si et seulement si $\mathfrak{g} \cong \mathfrak{n} \rtimes_\phi \mathfrak{h}$ pour un certain $\phi$.
\end{proposition}
...
Les deux premières classes d'algèbres de Lie que l'on considère sont les algèbres de Lie nilpotentes et résolubles. L'idée commune est de décrire des algèbres de Lie "presque abélienne" : ce sont exactement les algèbres de Lie qui s'obtiennent par extensions successives d'algèbres abéliennes. 
...
\begin{proposition}
    Une algèbre de Lie $\mathfrak{g}$ est nilpotente si et seulement si elle s'obtient comme suite finie d'extensions centrales  d'algèbres de Lie abéliennes, c'est à dire s'il existe une suite finie croissante d'idéaux $(\mathfrak{g}_i)_{0 \leq i\leq k}$ avec $\mathfrak{g}_0 = \mathfrak{g}$ et $\mathfrak{g}_k = 0$ telle que 
    \begin{diag}
        0 \arrow[r] & \mathfrak{g}_{i+1} \arrow[r] & \mathfrak{g}_i \arrow[r] & \mathfrak{g}_i/\mathfrak{g}_{i+1} \arrow[r] & 0
    \end{diag}
    est une extension centrale avec $\mathfrak{g}_i/\mathfrak{g}_{i+1}$ abélienne.
\end{proposition}
...
\begin{proposition}
    ~\begin{ilist}
        \item Toute sous-algèbre de Lie d'une algèbre de Lie nilpotente est nilpotente.
        \item Toute image directe d'une algèbre de Lie nilpotente par un morphisme d'algèbres de Lie est nilpotente.
        \item Toute extension d'algèbres de Lie nilpotentes est nilpotente
    \end{ilist}
\end{proposition}
...
Le théorème d'Engel donne une caractérisation utile du caractère nilpotent d'une algèbre de Lie. Plutôt que de vérifier que la série centrale descendante s'annule à partir d'un rang, ce qui peut être fastidieux en pratique, il suffit de vérifier que chaque élément agit de manière nilpotente via la représentation adjointe. La démonstration repose sur l'existence d'un vecteur propre commun à tous les éléments de l'algèbre de Lie nilpotente.

\begin{lemma}
    Soit $X\in \mathfrak{gl}(V)$ un endomorphisme nilpotent. Alors $\ad_X$ est aussi nilpotent.
\end{lemma}

\omitted

\begin{lemma}
    Soit $\mathfrak{g}$ une sous-algèbre de Lie de $\mathfrak{gl}(V)$ formée d'éléments nilpotent et où $V$ est un $\K$-espace vectoriel non nul de dimension finie. Alors il existe un vecteur non nul $v\in V$ tel que 
    $$X.v = 0 \quad \forall X \in \mathfrak{g}$$
\end{lemma}
...
\begin{theorem}
    ...
    $$\forall x\in \mathfrak{g}, \quad \ad_x \quad \text{est nilpotent}$$ 
\end{theorem}
...
\begin{proposition}
    Une algèbre de Lie $\mathfrak{g}$ est résoluble si et seulement si elle s'obtient comme suite finie d'extensions d'algèbres de Lie abéliennes, c'est à dire s'il existe une suite finie croissante d'idéaux $(\mathfrak{g}^i)_{0 \leq i\leq k}$ avec $\mathfrak{g}^0 = \mathfrak{g}$ et $\mathfrak{g}^k = 0$ telle que 
    \begin{diag}
        0 \arrow[r] & \mathfrak{g}^{i+1} \arrow[r] & \mathfrak{g}^i \arrow[r] & \mathfrak{g}^i/\mathfrak{g}^{i+1} \arrow[r] & 0
    \end{diag}
    est une extension avec $\mathfrak{g}^i/\mathfrak{g}^{i+1}$ abélienne.
\end{proposition}
...
\begin{proposition}
    ~\begin{ilist}
        \item Toute sous-algèbre de Lie d'une algèbre de Lie résoluble est résoluble.
        \item Toute image directe d'une algèbre de Lie résoluble par un morphisme d'algèbres de Lie est résoluble.
        \item Toute extension d'algèbres de Lie résolubles est résolubles.
        \item Toute sommes d'idéaux résolubles est résoluble.
    \end{ilist}
\end{proposition}
...
\begin{definition}
    Le \emph{radical} d'une algèbre de Lie $\mathfrak{g}$, noté $\mathcal{R}(\mathfrak{g})$, est le plus grand idéal résoluble de $\mathfrak{g}$.
\end{definition}

\subsection{Théorème de Lie}

Le théorème de Lie donne une trigonalisation simultanée des éléments de l'algèbre de Lie résoluble. À l'instar du théorème d'Engel, sa démonstration repose sur l'existence d'un vecteur propre commun.
...
\begin{theorem}
    Soit $\mathfrak{g}$ une sous-algèbre de Lie complexe et résoluble de $\mathfrak{gl}(V)$ où $V$ est un $\C$-espace vectoriel non nul de dimension finie. Alors les matrices des éléments de $\mathfrak{g}$ sont triangulaires supérieures. 
\end{theorem}
...
Ce critère donne une caractérisation pour vérifier la résolubilité d'une algèbre de Lie via l'annulation de sa forme de Killing. 

\begin{theorem}
    Soit $\mathfrak{g}$ une sous-algèbre de Lie complexe de $\mathfrak{gl}(V)$, où $V$ est un $\C$-espace vectoriel de dimension finie, telle que $\tr(xy)=0$ pour tout $x\in [\mathfrak{g}, \mathfrak{g}]$ et tout $y\in\mathfrak{g}$. Alors $\mathfrak{g}$ est résoluble.
\end{theorem}

\omitted

\begin{corollary}
    Soit $\mathfrak{g}$ une algèbre de Lie complexe telle que $\kappa(x, y) = 0$ pour tout $x \in [\mathfrak{g}, \mathfrak{g}]$ et tout $y \in \mathfrak{g}$. Alors $\mathfrak{g}$ est résoluble.
\end{corollary}
...
La troisième classe d'algèbres de Lie qui nous intéresse sont les algèbres de Lie dites semi-simples. 
...
\begin{definition}
    une \emph{algèbre de Lie semi-simple} est une algèbre de Lie $\mathfrak{g}$ telle que son radical $\mathcal{R}(\mathfrak{g})$ est nul.
\end{definition}

\begin{proposition}
    Soit $\mathfrak{g}$ une algèbre de Lie. Alors $\mathfrak{g}/\mathcal{R}(\mathfrak{g})$ est une algèbre de Lie semi-simple.
\end{proposition}
...
\begin{theorem}
    Soit $\mathfrak{g}$ une algèbre de Lie. Alors il existe une algèbre de Lie semi-simple $\mathfrak{h}$ telle que $\mathfrak{g} \cong \mathcal{R}(\mathfrak{g}) \rtimes \mathfrak{h}$.
\end{theorem}
